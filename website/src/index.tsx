...
<nav class="fixed top-0 left-0 right-0 z-50 border-b border-stone-200 dark:border-stone-800 bg-stone-50/90 dark:bg-stone-950/90 backdrop-blur-sm">
  <div class="max-w-6xl mx-auto px-8 flex items-center justify-between h-14">
    ...
    <div class="hidden md:flex items-center gap-6 text-xs tracking-widest uppercase">
      ...
      <a href="#projects" class="text-stone-900/60 dark:text-stone-50/60 hover:text-stone-900 dark:hover:text-stone-50 transition-colors">Projects</a>
    </div>
...
const PROJECTS = [
  {
    ...
    category: 'IoT & Cloud',
    status: null,
    description: 'Plataforma de analítica IoT desarrollada para un curso universitario. Responsable de toda la infraestructura cloud: procesamiento de imágenes con reconocimiento facial, redes virtuales y servidores en la nube.',
    tags: ['Amazon Rekognition', 'AWS EC2', 'AWS VPC', 'Cloud'],
    accent: '#003B8E',
    github: null,
    demo: null,
  },
  {
    ...
    status: null,
    description: 'Plataforma social de moda donde usuarios comparten fotos de outfits y la comunidad ayuda a encontrar dónde comprar cada prenda. Backend con Flask, base de datos PostgreSQL y almacenamiento en la nube.',
    tags: ['Python', 'Flask', 'Amazon S3', 'Amazon RDS', 'PostgreSQL'],
    accent: '#C8102E',
    github: 'https://github.com/candedefranco/WhereTheFit',
    demo: null,
  },
  {
    title: 'TruthChain',
    category: 'Cloud & IaC',
    status: 'In Progress',
    description: 'Sistema de verificación de contenido en tiempo real usando inteligencia artificial. Infraestructura como código con AWS CDK para despliegue automatizado y escalable.',
    tags: ['AWS CDK', 'Infrastructure as Code', 'TypeScript', 'AI'],
    accent: '#6B21A8',
    github: null,
    demo: null,
  },
  {
    ...
    category: 'Game Dev',
    status: null,
    description: 'Juego de estrategia estilo Tower Defense enfocado en programación orientada a objetos, patrones de diseño y lógica de juegos con múltiples niveles y enemigos.',
  ...
  },
  {
    ...
    status: null,
    description: 'Implementación de Ajedrez y Damas aplicando patrones de diseño, validación de reglas complejas, estructuras de datos y lógica de turnos.',
    tags: ['Java', 'Kotlin', 'OOP', 'Data Structures'],
    accent: '#2D6A4F',
    github: null,
    demo: null,
  },
...
const SectionProjects = () => html`
<section id="projects" class="border-b border-stone-200 dark:border-stone-800">
  <div class="max-w-6xl mx-auto px-4 md:px-8 py-24 md:py-32">
    <!-- Section label -->
    <div class="flex items-center gap-4 mb-20">
      <span class="text-xs font-mono font-medium text-stone-900/60 dark:text-stone-50/60">12</span>
      <span class="text-xs tracking-widest uppercase font-medium text-stone-900/80 dark:text-stone-50/80">Projects</span>
      <div class="flex-1 h-px bg-stone-300 dark:bg-stone-700"></div>
    </div>

    <div class="mb-16">
      <div class="w-8 h-px bg-[#C8102E] mb-8"></div>
      <h2 class="text-4xl md:text-5xl font-medium tracking-tight text-stone-900 dark:text-stone-50 leading-tight">
        Selected Work
      </h2>
      <p class="text-lg leading-relaxed text-stone-900/70 dark:text-stone-50/70 mt-4 max-w-[52ch]">
        A collection of projects spanning cloud infrastructure, web development, game design, and software engineering.
      </p>
    </div>

    <!-- Project cards grid -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-stone-200 dark:bg-stone-800">
      ${PROJECTS.map(({ title, category, status, description, tags, accent, github, demo }) => html`
      <div class="bg-stone-50 dark:bg-stone-950 p-8 flex flex-col gap-6 relative overflow-hidden group hover:bg-white dark:hover:bg-stone-900 transition-colors">
        <!-- Geometric accent: top border -->
        <div class="absolute top-0 left-0 right-0 h-0.5" style="background-color: ${accent}"></div>

        <!-- Header: category + status -->
        <div class="flex items-start justify-between">
          <span class="text-xs tracking-widest uppercase font-medium text-stone-900/50 dark:text-stone-50/50">${category}</span>
          ${status ? html`<span class="inline-block px-2 py-0.5 text-[11px] font-medium tracking-widest uppercase bg-[#6B21A8]/10 text-[#6B21A8] dark:text-[#7C3AED]">${status}</span>` : ''}
        </div>

        <!-- Title -->
        <div>
          <div class="w-6 h-px mb-4" style="background-color: ${accent}"></div>
          <h3 class="text-xl font-medium text-stone-900 dark:text-stone-50 leading-snug">${title}</h3>
          <p class="text-sm text-stone-900/70 dark:text-stone-50/70 leading-relaxed mt-3 max-w-[38ch]">${description}</p>
        </div>

        <!-- Tech tags -->
        <div class="flex flex-wrap gap-2 mt-auto pt-4">
          ${tags.map(tag => html`
          <span class="inline-block px-2 py-0.5 text-[11px] font-medium tracking-widest uppercase bg-stone-200 dark:bg-stone-800 text-stone-900/70 dark:text-stone-50/70">${tag}</span>`)}
        </div>

        <!-- Links -->
        <div class="border-t border-stone-200 dark:border-stone-800 pt-4 flex items-center gap-4">
          ${github ? html`
          <a href="${github}" target="_blank" rel="noopener" class="text-xs tracking-widest uppercase font-medium text-stone-900/60 dark:text-stone-50/60 hover:text-[#C8102E] transition-colors flex items-center gap-1.5">
            <span>GitHub</span><span>↗</span>
          </a>` : ''}
          ${demo ? html`
          <a href="${demo}" target="_blank" rel="noopener" class="text-xs tracking-widest uppercase font-medium text-stone-900/60 dark:text-stone-50/60 hover:text-[#C8102E] transition-colors flex items-center gap-1.5">
            <span>Demo</span><span>↗</span>
          </a>` : ''}
          ${!github && !demo ? html`
          <span class="text-xs tracking-widest uppercase text-stone-900/30 dark:text-stone-50/30">Private repository</span>` : ''}
        </div>
      </div>`)}
    </div>

    <!-- CTA to GitHub profile -->
    <div class="mt-16 flex items-center gap-6 border-t border-stone-200 dark:border-stone-800 pt-8">
      <a href="https://github.com/candedefranco" target="_blank" rel="noopener" class="px-6 py-3 bg-stone-900 dark:bg-stone-50 text-stone-50 dark:text-stone-900 text-sm font-medium tracking-wide hover:bg-stone-800 dark:hover:bg-stone-200 active:scale-[0.98] transition-all duration-150">
        View all on GitHub ↗
      </a>
      <span class="text-sm text-stone-900/50 dark:text-stone-50/50">More projects and contributions</span>
    </div>
  </div>
</section>`
